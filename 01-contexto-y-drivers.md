...
**HECHO VERIFICADO (anteproyecto):** El objetivo funcional es generar recetas personalizadas usando exclusivamente los ingredientes disponibles en la nevera del usuario, dirigido a familias de ingresos medios en Bogotá.

**DECISIÓN DE EQUIPO (confirmada):** La arquitectura de microservicios del anteproyecto se documenta como Roadmap / Trabajo futuro, no como parte del alcance evaluado en este dossier. El alcance de Semana 1-4 es el sistema real: HTML/CSS/JS estático + Firebase.

## 2. Contexto

**HECHO VERIFICADO (anteproyecto, FAO 2019):** El proyecto responde a que una proporción significativa de los residuos sólidos en Bogotá corresponde a alimentos aún no vencidos, y a tres causas identificadas por el equipo: falta de planificación de comidas, desconocimiento de recetas con ingredientes disponibles, y compras impulsivas.
...
| Profesor / evaluador del curso de Arquitectura | Evidencia verificable en Git de decisiones arquitectónicas reales | El anteproyecto documenta arquitectura no implementada — riesgo directo aquí |
| Equipo desarrollador | Sistema simple de mantener sin backend propio | Tensión con el plan de pruebas extenso del anteproyecto, que asume backend real |
| Firebase (proveedor externo) | — | Dependencia total: sin Firebase, el sistema no tiene backend ni autenticación |

<!-- Falta: confirmar si los competidores (SuperCook, Allrecipes) mencionados en el anteproyecto
...
**SUPUESTO:** El anteproyecto asume MySQL/PostgreSQL, Redis y un Motor de IA (Claude) como parte de la arquitectura — esto no está confirmado como restricción real, sino como diseño propuesto no implementado.

## 5. Drivers arquitectónicos preliminares

**HECHO VERIFICADO (anteproyecto, RNF02):** El equipo ya documentó Seguridad como requisito no funcional explícito: protección de datos personales, cumplimiento de normativa colombiana, política de privacidad.

<!-- No elijo el resto de drivers por ustedes. Discutan en equipo cuáles de estos RNF ya
     definidos (RNF01 usabilidad, RNF03 disponibilidad, RNF04 rendimiento, RNF05 compatibilidad,
     RNF06 mantenibilidad) se consideran drivers arquitectónicos reales del sistema ACTUAL
     (no del sistema aspiracional de microservicios) -->

## 6. Riesgos iniciales

**RIESGO VERIFICADO:** El sistema real no tiene tests automatizados, a pesar de que el anteproyecto documenta un plan de QA extenso (pruebas unitarias, integración, rendimiento, seguridad) que no se ha ejecutado. Esto es evidencia faltante crítica para el checkpoint de Semana 2.
...
- Se asume que las recetas mostradas son estáticas/curadas por el equipo, no generadas por IA en tiempo real (el anteproyecto menciona un "Motor IA / Claude" en el diagrama de secuencias que no está confirmado como implementado en el sistema real).
